 ...
 * 잔차(remainder)로 따로 계산해 흡수자에게 몰아준다. (기획설계 4.1/4.3)
 */
 ...
 * 같은 개념의 변주이므로 하나로 통합한다. `full`은 value가 항목 전액인
 * 특수 케이스이며 UI에서만 "전액" 토글로 따로 노출한다. (기획설계 3.4)
 */
export interface ExtraCharge {
  participantId: string
  type: 'amount' | 'full'
  /** type이 'amount'일 때만 사용. 정수 원 단위. */
  value?: number
}
...
export interface Settlement {
   ...
   * 예외인 항목만 payerId를 바꾼다. (기획설계 10 — 열린 질문 1)
   */
