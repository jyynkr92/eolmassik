 ...
 * 입력칸에 띄울 오류 문구.
 *
 * 빈 이름은 오류로 보여주지 않는다. 아무것도 안 쓴 처음 상태가 곧 빈 이름이라, 화면에
 * 들어오자마자 빨간 글씨를 보게 된다. 못 누른다는 건 비활성 버튼이 이미 말해 준다.
 */
const getErrorMessage = (check: ParticipantNameCheck): string | undefined => {
  if (check.isValid || check.error === 'empty') return undefined;

  return PARTICIPANTS_TEXT.nameError[check.error];
};
...
const ParticipantForm = ({ participants, onAdd }: Props) => {
  ...
  const errorMessage = getErrorMessage(check);
